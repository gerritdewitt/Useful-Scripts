...
# 2016-04-11, 2016-05-11, 2016-06-03.
...
# Sources:
declare -x ALERTUS_PB_APP_NAME="Desktop Activator.app"
declare -x ALERTUS_PB_LAUNCH_AGENT_NAME="com.alertus.DesktopActivator.plist"
...
declare -x PR_APP_SUP_DIR="$PACKAGE_ROOT_DIR/Library/Application Support/Alertus Technologies/Desktop Activator"

# MARK: gather_materials()
# Sources installer materials.
function gather_materials(){
echo "This script builds a proper installer package for the Alertus Panic Button, including properly wrapping the configuration within the installer package.  The resulting package is suitable for mass deployment via Munki, Casper, Absolute Manage, etc."
echo "\nSource Materials"
echo "Provide the full path to the Alertus Panic Button (aka Desktop Activator) DMG containing an unmodified PKG:"
read SRC_ALERTUS_INSTALLER_DMG

echo "Provide the full path to the DesktopActivator.org.properties file:"
read ALERTUS_PB_CONFIG_FILE_PATH
...
PACKAGE_PATH="$THIS_DIR/Alertus_Panic_Button-$PACKAGE_VERSION.pkg"
}
...
cp -R "$TEMP_DIR/$ALERTUS_PB_APP_NAME" "$PR_APPS_DIR/$ALERTUS_PB_APP_NAME" && echo "Copied app to Applications directory in package root."
# Copy launch agent into package root:
cp "$THIS_DIR/$ALERTUS_PB_LAUNCH_AGENT_NAME" "$PR_LA_DIR/$ALERTUS_PB_LAUNCH_AGENT_NAME" && echo "Copied launch agent to LaunchAgents directory in package root."
# Copy config file into package root:
cp "$ALERTUS_PB_CONFIG_FILE_PATH" "$PR_APP_SUP_DIR/DesktopActivator.org.properties" && echo "Copied config file to App Support directory in package root."
...
chmod 0644 "$PACKAGE_ROOT_DIR/Library/LaunchAgents/$ALERTUS_PB_LAUNCH_AGENT_NAME" && echo "Set permissions on launch agent in package root."
